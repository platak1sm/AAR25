...
public class QuestionUI : MonoBehaviour
{
    ...
    private void ShowLeaderboard()
    {
        ...
        LeaderboardManager.Instance.GetTopScores((entries) =>
        {
            ...
            {
                if (entries[i].name != playerTeamName) leaderboardDisplay.AppendLine($"{i + 1}. {entries[i].name} | {entries[i].score} points");
                else leaderboardDisplay.AppendLine($"{i + 1}. {entries[i].name} | {entries[i].score} points <b><-- You</b>");
            }
